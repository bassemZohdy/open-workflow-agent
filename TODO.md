...
- [x] Run acceptance on a real Kubernetes cluster, including timeout/cancellation/restart cleanup, retained-log secret safety, and namespace/RBAC enforcement. (2026-08-28 on kind v0.33.0 / Kubernetes 1.37 with Calico for NetworkPolicy enforcement: end-to-end `run.container` execution, timeout -> `sandbox_timeout` with Job cleanup, cancellation -> `cancelled` with Job cleanup, ambiguous failure bounded by `activeDeadlineSeconds` + TTL with zero residue, secrets delivered via `secretKeyRef` and never present in responses/logs, RBAC verified allow + six forbidden paths including secrets/cross-namespace/cluster-scope, egress denial enforced. Findings fixed: `run.container` gate now admits the Kubernetes backend, Job pods run as numeric non-root 65532:65532, deadline failures map to `sandbox_timeout`, controller log reads are best-effort on failed Jobs, runtime posts execution JSON with the correct content type. Reproduction: docs/development.md "Kubernetes acceptance with kind".)
- [ ] Run OpenShift acceptance for SCC/security-context/arbitrary-UID behavior and the same lifecycle/security cases.
- [x] Advertise Kubernetes/OpenShift container execution through `/v1/capabilities` only after the relevant deterministic and deployment acceptance gates are green. (`features.sandbox` reports the kubernetes backend with container execution only when deployment-enabled and its acceptance gates are green -- verified live; OpenShift advertisement additionally requires the OpenShift acceptance above.)
